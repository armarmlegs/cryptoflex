  ...
  render() {
    ...
    return (
      ...
      <div class="crypto-dets">
        ...
        {this.state.crypto.map(({name, price, rank, id, logo_url}) => {
          return (
            <tr key={id}>
              <td><img class="logo-home" src={logo_url} alt="" /></td>
              <td><h2>{name} </h2></td>
              <td><h3>{price}</h3></td>
              <td> <h5>{rank}</h5></td>
             
              <td> <button class="link-home"> <Link to={`/crypto/${id}`}>See more</Link></button> </td> 
              </tr>
          );
        })}
        </table>
