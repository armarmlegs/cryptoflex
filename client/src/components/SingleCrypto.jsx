...
class SingleCrypto extends Component {
    ...
    state = {
      ...
      editFaves = (e) =>{
        // recuperer l'id de la crypto en question
        const id = this.props.match.params.id;
      
        api.addFave({favoriteCryptos: id}).then(res => {
          console.log(res)
        })
        //  faire le patch avec un object de la forMe : {crypto : la valeur"}
        // dans le then de la reponse, on recupere le user
        // on veut Mettre le user a jour dans le context
        // (withUser ; )
        // regarder les props et voir ce qui nous perMet de Mettre ç jour le user 
        // dans le contexte
        // open > UserProvider.jsx
        }
      ...
      <div class="crypto-dets"> 
       <p class="crypto-header-bar"><Link class="crypto-header-bar-hover" to={`/crypto/`}>All the cryptocurrency</Link> > {this.state.crypto.filter(curr => curr.id === id)[0].id}</p>
        ...
        <div class="crypto-all-chart4">
         ...
         <div class="button-harm">
         <button class="link-home"><Link to={`/api/articles`}> <p>More Infos about {this.state.crypto.filter(curr => curr.id === id)[0].name}</p></Link></button>
         </div>
         <div class="button-harm">
         <button class="link-home" onClick ={this.editFaves}>Add to favorites</button>
         </div>
         </div>
